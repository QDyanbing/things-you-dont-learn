...
export class FileCoordinator {
  ...
  getLastQueuedChunkIndex(): number | null {
    ...
    return queuedChunkIndexes.length > 0
      ? queuedChunkIndexes[queuedChunkIndexes.length - 1]
      : null;
  }
   ...
   *
   * This follows the same broad scheduling view as `getPendingChunkIndexes()`
   * and includes both `PENDING` and retryable `ERROR` chunks.
   */
   ...
   *
   * This uses the same broad scheduling view as `getPendingChunkIndexes()`.
   */
   ...
   *
   * This checks only chunks currently marked as `UPLOADING`; it does not infer
   * upload activity from the coordinator status alone.
   */
  ...
  getLastUploadingChunkIndex(): number | null {
    ...
    return uploadingChunkIndexes.length > 0
      ? uploadingChunkIndexes[uploadingChunkIndexes.length - 1]
      : null;
  }
  ...
  /**
   * Returns the first chunk index currently marked as failed.
   *
  ...
  getLastFailedChunkIndex(): number | null {
    ...
    return failedChunkIndexes.length > 0
      ? failedChunkIndexes[failedChunkIndexes.length - 1]
      : null;
  }
