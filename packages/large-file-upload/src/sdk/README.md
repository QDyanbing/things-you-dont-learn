...
- 空文件也会保留一个零字节分片，方便调用方复用同一套分片查询和上传流程
...
const failedByteSize = coordinator.getFailedByteSize();
...
`canUpload()` 是开始上传前的轻量判断，适合控制上传按钮；它要求已经完成 `prepare()`、没有活跃上传任务，并且当前仍存在待调度分片。

真正执行时仍应调用 `upload()`，由 SDK 内部复用或校验当前上传任务。`upload()` 使用待调度口径，会处理 `PENDING` 和可重试的 `ERROR` 分片。
...
`getQueuedByteSize()` 是严格等待上传列表的字节视角，只统计当前 `PENDING` 分片，不包含失败后等待重试的 `ERROR` 分片。
...
`getFailedByteSize()` 是失败分片列表的字节视角，只统计当前本地状态为 `ERROR` 的分片大小，适合展示失败体量或估算重试成本。
...
按分片下标读取的方法通常不会因为无效下标抛错；不存在或尚未准备的分片会返回 `null` 或 `false`。真正发起上传的方法会在前置条件不满足时抛出错误，便于调用方进入失败处理。
...
| `getPreparedByteSize()` | 获取当前已准备分片的合计字节数；在调用 `prepare()` 前默认是 `0` | `number` |
| `getPreparedChunkIndexes()` | 获取当前已准备分片的下标列表；在调用 `prepare()` 前默认返回空数组 | `number[]` |
...
| `getQueuedChunkCount()` | 获取当前严格等待上传的分片数量；当前和 `getQueuedChunkIndexes().length` 口径一致 | `number` |
| `getQueuedByteSize()` | 获取当前严格等待上传分片的合计字节数；当前只统计状态为 `PENDING` 的分片 | `number` |
...
| `getLastPendingChunkIndex()` | 获取最后一块仍需要进入上传流程的分片下标；没有待调度分片时返回 `null` | `number \| null` |
...
| `getFailedByteSize()` | 获取当前上传失败分片的合计字节数；当前只统计状态为 `ERROR` 的分片 | `number` |
